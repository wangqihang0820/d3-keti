...
class SetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        knn = KNN(k=self.nsample, transpose_mode=True)
        _, idx = knn(xyz, new_xyz)
        
        # ★ 新增这一行：将 64 位 LongTensor 强转为 32 位 IntTensor
        idx = idx.int()
        ...
        for i, conv in enumerate(self.mlp_convs):
            grouped_features = nn.functional.relu(self.mlp_bns[i](conv(grouped_features)))
        new_points = torch.max(grouped_features, -1)[0].transpose(1, 2).contiguous()
...
class FeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        interpolated_points = torch.sum(
            pointnet2_utils.grouping_operation(points2.transpose(1, 2).contiguous(), idx) * weight.unsqueeze(1), dim=-1
        ).transpose(1, 2).contiguous()
        return interpolated_points

class BottleneckSkipFusion(nn.Module):
    def __init__(self, in_dim, skip_dim, out_dim, compression_ratio=8):
        super().__init__()
        self.bottleneck = nn.Sequential(
            nn.Linear(skip_dim, skip_dim // compression_ratio),
            nn.GELU(),
            nn.Linear(skip_dim // compression_ratio, skip_dim)
        )
    ...
    def forward(self, x_dec, x_enc):
        x_enc_compressed = self.bottleneck(x_enc)
        x_cat = torch.cat([x_dec, x_enc_compressed], dim=-1)
        return self.fusion_mlp(x_cat)
...
class ReconNet3D(nn.Module):
    def __init__(self, in_dim=768):
        ...
        self.enc_block1 = Block(self.dim, num_heads=6)
        self.sa1 = SetAbstraction(256, 16, self.dim, [self.dim, self.dim])
        
        self.enc_block2 = Block(self.dim, num_heads=6)
        self.sa2 = SetAbstraction(64, 16, self.dim, [self.dim, self.dim])
        
        self.bottleneck_block = Block(self.dim, num_heads=6)
        
        self.fp2 = FeaturePropagation()
        self.fusion2 = BottleneckSkipFusion(in_dim=self.dim, skip_dim=self.dim, out_dim=self.dim, compression_ratio=8)
        self.dec_block2 = Block(self.dim, num_heads=6)
        
        self.fp1 = FeaturePropagation()
        self.fusion1 = BottleneckSkipFusion(in_dim=self.dim, skip_dim=self.dim, out_dim=self.dim, compression_ratio=8)
        self.dec_block1 = Block(self.dim, num_heads=6)
        
        self.out_proj = nn.Linear(self.dim, self.dim)

    def forward(self, x, xyz): 
        x_e1 = self.enc_block1(x)
        xyz_d1, x_d1 = self.sa1(xyz, x_e1)
        
        x_e2 = self.enc_block2(x_d1)
        xyz_d2, x_d2 = self.sa2(xyz_d1, x_e2)
        
        x_bot = self.bottleneck_block(x_d2)
        
        x_u2 = self.fp2(xyz_d1, xyz_d2, x_e2, x_bot)
        x_f2 = self.fusion2(x_u2, x_e2)
        x_dec2 = self.dec_block2(x_f2)
        
        x_u1 = self.fp1(xyz, xyz_d1, x_e1, x_dec2)
        x_f1 = self.fusion1(x_u1, x_e1)
        x_out = self.dec_block1(x_f1)
...
# import torch
# import torch.nn as nn
# from models.models import Block  # 复用你原本的 Transformer Block

# class ReconNet3D(nn.Module):
#     """
#     符合论文描述的 "Transformer 重建网络"
#     采用严格的 Encoder-Decoder (Bottleneck) 架构，保留自编码器的“压缩-重建”异常过滤特性。
#     坚决不使用破坏物理拓扑的 2D 卷积与 view 操作。
#     """
#     def __init__(self, in_dim=768, embed_dim=384, encoder_depth=3, decoder_depth=3):
#         super().__init__()
#         self.in_dim = in_dim
#         self.embed_dim = embed_dim
        
#         # ==========================================
#         # 1. 编码器 (Encoder) - 构建信息瓶颈
#         # ==========================================
#         # 降维投影：强制压缩信息，丢弃高频的“异常噪声”
#         self.enc_proj = nn.Linear(in_dim, embed_dim)
        
#         # Transformer 编码：在潜空间中提取纯净的全局/局部几何拓扑
#         self.encoder = nn.ModuleList([
#             Block(embed_dim, num_heads=6)
...
#         # ==========================================
#         # 升维投影：从紧凑的潜空间映射回原始维度
#         self.dec_proj = nn.Linear(embed_dim, in_dim)
        
#         # Transformer 解码：结合注意力机制，精准重建出无异常的健康特征
#         self.decoder = nn.ModuleList([
#             Block(in_dim, num_heads=8)
#             for _ in range(decoder_depth)
#         ])
#         self.dec_norm = nn.LayerNorm(in_dim)

#     def forward(self, x):
#         # x: [B, 1024, 768] (B, N, C)
        
#         # --- 编码阶段 (压缩) ---
#         latent = self.enc_proj(x)       # [B, 1024, 384]
#         for blk in self.encoder:
#             latent = blk(latent)
#         latent = self.enc_norm(latent)  # 纯净的几何潜变量
        
#         # --- 解码阶段 (重建) ---
#         reconstructed = self.dec_proj(latent) # [B, 1024, 768]
#         for blk in self.decoder:
#             reconstructed = blk(reconstructed)
#         out = self.dec_norm(reconstructed)
        
#         return out # [B, 1024, 768] 完美对齐的输出
